...
fn expand_schedule(args: ScheduleArgs, item_impl: ItemImpl) -> syn::Result<TokenStream2> {
    ...
    let expanded = quote! {
        ...
        impl #struct_ty {
            /// Returns the global [`::lib_trigger::CancellationToken`] for this scheduler.
            /// Cancel it to stop `run()` or `run_until_signal()`.
            pub fn shutdown_token() -> ::lib_trigger::CancellationToken {
                static TOKEN: ::std::sync::OnceLock<::lib_trigger::CancellationToken> =
                    ::std::sync::OnceLock::new();
                TOKEN.get_or_init(|| ::lib_trigger::CancellationToken::new()).clone()
            }

            /// Build and run the scheduler until the iterator is exhausted or
            /// [`shutdown_token()`](Self::shutdown_token) is cancelled.
            pub async fn run() {
                let iter = ::lib_schedule::time::SpecIteratorBuilder::new(
                    #spec_lit,
                    ::chrono::Utc,
                )
                .build()
                ...
                let mut scheduler = ::lib_trigger::Scheduler::new(iter, Self::#on_error_ident)
                    .with_shutdown_token(Self::shutdown_token());

                #fire_on_start_call

                for entry in ::lib_trigger::inventory::iter::<::lib_trigger::JobEntry>() {
        ...
        impl ::lib_trigger::ScheduleErrorHandler<#error_type> for #struct_ty {
...
fn expand_job(struct_path: Path, item_fn: ItemFn) -> syn::Result<TokenStream2> {
    ...
    let helper_ident = quote::format_ident!("__lib_trigger_job_{}", fn_ident);

    let expanded = quote! {
        #item_fn

        // Plain fn so its pointer is a const expression (required by inventory::submit!).
        fn #helper_ident() -> ::lib_trigger::BoxedFuture {
            ::std::boxed::Box::pin(async move {
                if let Err(e) = #fn_ident().await {
                    <#struct_path as ::lib_trigger::ScheduleErrorHandler<#error_type>>::handle_error(e).await;
                }
            })
        }

        ::lib_trigger::inventory::submit! {
            ::lib_trigger::JobEntry {
