...
impl FromStr for Cycle {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        let caps = cycle_re.captures(s).ok_or(Error::ParseError)?;
        let num = caps
            .name("num")
            .unwrap()
            .as_str()
...
impl FromStr for DayKind {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        let caps = daykind_re.captures(s).ok_or(Error::ParseError)?;
...
impl FromStr for BusinessDayStep {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        let caps = re.captures(s).ok_or(Error::ParseError)?;
...
impl FromStr for DateSpec {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        let caps = &spec_re.captures(s).ok_or(Error::ParseError)?;
        let intervals = caps
            .iter()
            .skip(1)
            .take(3)
...
mod tests {
    ...
    #[test]
    fn test_one() {
        let spec = DateSpec::from_str("3Y:4M:7B:31LN").unwrap();
